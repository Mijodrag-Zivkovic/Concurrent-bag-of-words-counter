...
public class FileInputView {
	...
	public FileInputView(FileInput fileInput, MainView mainView) {
		...
		scanButton = new Button("Resume");
		scanButton.setOnAction(e -> scanResume());
		scanButton.setMinWidth(width/2);
		scanButton.setMaxWidth(width/2);
		scanButton.setDisable(true);
		//VBox.setMargin(scanButton, new Insets(15, 0, 0, 0));
		//main.getChildren().add(scanButton);

		pauseScanButton = new Button("Pause");
		pauseScanButton.setOnAction(e -> pauseScan());
		pauseScanButton.setMinWidth(width/2);
		pauseScanButton.setMaxWidth(width/2);
		pauseScanButton.setDisable(true);
		//VBox.setMargin(pauseScanButton, new Insets(15, 0, 0, 0));
		//main.getChildren().add(pauseScanButton);

		hbox2.getChildren().add(scanButton);
		hbox2.getChildren().add(pauseScanButton);
		...
		fileInputWorker = new FileInputWorker(fileInput,directories.getItems(),status);
...
//////////////////////////////////////////////////////////////////////////////
	private void linkCruncher(Cruncher cruncher) {
		...
		fileInputWorker.getQueuesList().add(cruncher.getQueue());
	...
	public void removeLinkedCruncher(Cruncher cruncher) {
		...
		fileInputWorker.getQueuesList().remove(cruncher.getQueue());
		updateLinkCruncherButtonEnabled();
	}
	...
	private void addDirectory() {
		...
		if (fileDirectory != null && fileDirectory.exists() && fileDirectory.isDirectory()) {
			...
			//concurrent list add directory
		}
	}

	private void removeDirectory(Directory directory) {
		///
		fileInputWorker.deleteDir(directory);
		directories.getItems().remove(directory);
	}

	private void start() {
//		//Thread t = new Thread(new Probni());
//		Thread t = new Thread(fileInputWorker);
//		t.start();
		Pools.fileInputWorkers.add(fileInputWorker);
		Pools.fileInputPool.submit(fileInputWorker);
		start.setDisable(true);
		pauseScanButton.setDisable(false);
		//Pools.fileInputPool.submit(new FileInputWorker(fileInput,directories.getItems()));
	}

	private void removeDiskInput() {
		Pools.fileInputWorkers.remove(fileInputWorker);
		mainView.removeFileInputView(this);
		fileInput.setRunning(false);
	}

	private void scanResume(){
		fileInputWorker.setPaused(false);
		fileInputWorker.resume();
		pauseScanButton.setDisable(false);
		scanButton.setDisable(true);
	}

	private void pauseScan(){
		fileInputWorker.setPaused(true);
		pauseScanButton.setDisable(true);
